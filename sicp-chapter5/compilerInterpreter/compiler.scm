;; this is the compiler from ex5-42.scm
(load "lexical-address-operations.scm") ;; loading lexical address operations and find-variable procedure

(define (compile exp target linkage cte)
	(cond 	((self-evaluating? exp)
			(compile-self-evaluating exp target linkage))
		((primitive=? exp)
			(compile-= exp target linkage cte))
		((primitive+? exp)
			(compile-+ exp target linkage cte))
		((primitive-? exp)
			(compile-- exp target linkage cte))
		...
		((let? exp)
			(compile (let->combination exp) target linkage cte))
		((let*? exp) 
			(compile (let*->nested-lets exp) target linkage cte))
		((application? exp)
...
(define (compile-variable exp target linkage cte)
			...
			(end-with-linkage linkage
				(append-instruction-sequences
					...
					(make-instruction-sequence '() '(env)
						'((assign env (op get-global-environment)))) ;; if the address is not found, the variable can still be in the global environment, because it's accessible only on run-time, not on compilation time...so on run time we will need to look it up
...
(define (compile-assignment exp target linkage cte)
...
;; *********************************************************
;; *********************************************************
;; 		PROCEDURES FOR OPEN-CODED PRIMITIVES
(define (spread-arguments operands cte)
	(if (= 2 (length operands))     
		(preserving '(env)
			(compile (car operands) 'arg1 'next cte)
				(preserving '(arg1)
					(compile (cadr operands) 'arg2 'next cte)
					(make-instruction-sequence
					'(arg1) '() '())))
	(error "Spread-arguments expects 2 args -- COMPILE" operands)))		
	
(define (compile-2-args-primitive-operation operator exp target linkage cte)
	(end-with-linkage linkage	;; create an instruction that ends with the proper linkage
			(append-instruction-sequences		;; we will append spread-arguments with the primitive operation itself
				(spread-arguments (operands exp) cte) ;; first we spread the operands on the argument registers
...
(define (compile-= exp target linkage cte)
	(compile-2-args-primitive-operation '= exp target linkage cte))
