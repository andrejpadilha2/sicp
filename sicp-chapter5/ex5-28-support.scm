;; Just copying the eceval, and then notating with *** the place where I change it
...
(load "cap5-RegisterMachineSimulator.scm") ;; the simulator is responsible of simulating eceval as a register machine
...
(define eceval ;; EXPLICIT CONTROL EVALUATOR MACHINE
	(make-machine
		'(exp env val proc argl continue unev) ;; all register of the ECE
		(list ;; all operations considered primitives of the ECE
			...
			(list 'no-more-exps? no-more-exps?))						;; ***************** ADDING ONE MORE OPERATION
		'(
		;; DRIVER-LOOP
