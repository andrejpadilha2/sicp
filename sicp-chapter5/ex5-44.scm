;; this solution is definitely wrong
	...
	(end-with-linkage linkage	;; create an instruction that ends with the proper linkage
			(append-instruction-sequences		;; we will append spread-arguments with the primitive operation itself
				...
				(make-instruction-sequence '(arg1 arg2) (list target)
					`((assign ,target (perform (op ,operator) (reg arg1) (reg arg2)))))))) ;; the instruction is the direct application of the proper primitive operator
...
(define (compile-= exp target linkage cte)
	(let* ((operator (operator exp))
		(variable-address (find-variable operator cte)))
		(if (eq? variable-address 'not-found)
			(compile-2-args-primitive-operation '= exp target linkage cte) ;; normal primitive operator
			(compile-2-args-primitive-operation (lexical-address-lookup variable-address cte) exp target linkage cte))))
